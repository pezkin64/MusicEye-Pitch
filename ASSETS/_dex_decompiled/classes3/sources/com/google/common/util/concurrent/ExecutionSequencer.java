...
public final class ExecutionSequencer {
    ...
    public <T> ListenableFuture<T> submitAsync(final AsyncCallable<T> asyncCallable, final Executor executor) {
        ...
        AnonymousClass2 r0 = new AsyncCallable<T>() {
            public ListenableFuture<T> call() throws Exception {
                if (!zap$.ExternalSyntheticBackportWithForwarding0.m(atomicReference, RunningState.NOT_RUN, RunningState.STARTED)) {
        ...
        AnonymousClass4 r02 = new Runnable() {
            public void run() {
                if (submitAsync.isDone() || (nonCancellationPropagating.isCancelled() && zap$.ExternalSyntheticBackportWithForwarding0.m(atomicReference, RunningState.NOT_RUN, RunningState.CANCELLED))) {
